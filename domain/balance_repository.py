"""
domain/balance_repository.py — CRUD for balance_snapshots.
"""
...
async def get_snapshot(
    ...
    q = (
        ...
        .where(BalanceSnapshot.especialist_code == especialist_code)
...
async def ensure_snapshot(
    ...
    existing = await get_snapshot(session, especialist_code, work_window_id)
    if existing:
        return existing
    ...
    await session.flush()
    logger.info(
        "Created balance_snapshot especialist=%s window=%s initial_balance=%s",
        especialist_code, work_window_id, initial_balance,
    )
    return row
